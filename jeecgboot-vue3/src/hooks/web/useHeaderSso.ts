...
export function redirectToHeaderSsoSignOut(redirectUrl = getHeaderSsoHomeUrl()) {
  const keycloakLogoutPath = `/__sso/keycloak-logout?rd=${encodeURIComponent(redirectUrl)}`;
  window.location.href = `/oauth2/sign_out?rd=${encodeURIComponent(keycloakLogoutPath)}`;
}
