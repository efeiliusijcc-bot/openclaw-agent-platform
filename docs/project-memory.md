...
## 2026-06-22 - Skill Review / Publish Lifecycle Progress

- Goal: publish a formal Skill only from a reviewed immutable Draft Version snapshot. The implementation commit is `a0839b3e feat(openclaw): add skill review publish lifecycle`, pushed to `openclaw-platform/main`, and `/opt/openclaw-build/jeecgboot` on `116.204.135.83` has been pulled to that commit.
- Backend additions: `OpenclawSkillReview`, `OpenclawSkillReviewMapper`, `IOpenclawSkillReviewService`, `OpenclawSkillReviewServiceImpl`, `OpenclawSkillReviewController`, submit/action DTOs, and `OpenclawSkillReviewVO`. New endpoints: `POST /openclaw/skill/draft/{draftId}/versions/{versionNo}/submit-review`, `GET /openclaw/skill/reviews`, `GET /openclaw/skill/reviews/{reviewId}`, `POST /openclaw/skill/reviews/{reviewId}/approve`, `POST /openclaw/skill/reviews/{reviewId}/reject`, and `POST /openclaw/skill/reviews/{reviewId}/cancel`.
- Published version additions: `OpenclawPublishedSkillVersion`, `OpenclawPublishedSkillVersionMapper`, and `V3.9.2_25__openclaw_skill_review_publish.sql` for both MySQL and PostgreSQL. Production Flyway is disabled historically, so the MySQL migration was applied manually on server DB `jeecg-boot`; `openclaw_skill_review` and `openclaw_published_skill_version` were verified to exist.
- Submit Review validation: caller must have draft access; version must exist; version must have `lintStatus=lint_passed` and `testStatus=success`; the bound `OpenclawSkillTestRun` must be successful and match `draftVersionNo/fileHash`; duplicate unfinished review for the same draft/version is rejected; review records persist `fileSnapshotJson`, `testReportJson`, and `aiRecordIdsJson` instead of storing references only.
- Review actions: approve/reject require reviewer/admin permission; cancel requires submitter/admin; terminal statuses `APPROVED/REJECTED/CANCELLED` cannot be operated again; `reviewComment` is saved and all operations write OpenClaw audit logs.
- Publish rule: approve publishes only from `review.fileSnapshotJson`, never from the current draft directory. It writes the official Skill directory, creates/updates the latest `openclaw_skill`, creates `openclaw_published_skill_version`, keeps published versions isolated from Draft Runtime, does not reuse `skill_draft_test_*` agent keys, and returns `publishedSkillId` / `publishedVersionNo`. Old direct draft submit/approve/reject/publish service entry points now fail with guidance to use the fixed-version Review flow.
- Frontend additions: `src/views/openclaw/api.ts` adds Review APIs; `SkillEditor.vue` shows a submit-review action for lint/test-passed versions, supports `submitComment`, and displays `reviewStatus`; `SkillDraftList.vue` removes old direct submit/approve/reject/publish actions; `SkillReviewList.vue` is now a Review list/detail page showing immutable snapshot, diff, Test Report, AI Edit/Repair records, approve/reject actions, and `publishedVersionNo` after approval.
- Verification: local backend `mvn -pl jeecg-boot-module/jeecg-module-demo -am -DskipTests compile` passed; server backend `mvn -pl jeecg-module-system/jeecg-system-start -am -DskipTests package` passed; server frontend `dist/index.html` was updated at `2026-06-22 08:24` with 465 assets; the built jar was copied to `/opt/openclaw-jeecg/backend/app.jar`; frontend dist was synced to `/opt/openclaw-jeecg/frontend/html/`; `http://116.204.135.83/` and `http://116.204.135.83:8081/jeecg-boot/sys/randomImage/0` both returned HTTP 200 after deployment.
- Server acceptance: `cd /opt/openclaw-build/jeecgboot && python3 scripts/openclaw_skill_draft_closed_loop_acceptance.py` passed with top-level `success=true` on 2026-06-22. Key Review/Publish steps passed: `review-submit` returned HTTP 200 with `status=SUBMITTED` and a fixed snapshot; `review-detail-before-approve` returned files and Test Report; `review-approve-publish` returned HTTP 200 with `publishedSkillId=2068959143514980353` and `publishedVersionNo=1`; `review-mutate-draft-after-publish` succeeded; `published-snapshot-isolated` confirmed both review snapshot and published snapshot were unchanged after draft mutation. The same run also passed invoice AI Edit, Echo Lint/Test/report, version diff/rollback, failed-test AI Repair preview/apply, and Gateway log checks.
- Deployment note: the first server acceptance attempt after deployment returned 500 for Review endpoints. The visible response was a secondary Spring/Tomcat error `NoClassDefFoundError: org/springframework/web/servlet/ModelAndViewDefiningException`, which masked the original failure. After rebuilding/redeploying the backend jar and restarting `openclaw-jeecg-backend`, replaying submit-review for the same tested version returned HTTP 200 and the full acceptance script passed. Keep the service-level submit-review error log in place to avoid losing the real cause if this classpath/error-page issue recurs.
- Environment notes: local frontend `pnpm build` failed on Node `v24.12.0` after Vite transformed modules with exit code `1073807364`; existing project notes already mark the frontend toolchain as unstable on Node 24, while the server build artifact exists. SSH to `116.204.135.83` intermittently closes long or compound commands, so prefer short commands or a single server-side script for final acceptance.
